#!/usr/bin/env python3
"""Collect public scene snapshots and physically separate evaluator labels.

The protocol YAML owns every scenario and split choice.  This runner contains
no benchmark, object, seed-block, or version-specific branch.
"""
...
def visible_pixels(env, observation: dict, *, camera: str, instance: str) -> int:
    """Count evaluator-only instance pixels without exposing them to policy data."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-closed",
        action="store_true",
        help="explicitly open a protocol split whose open_now field is false",
    )
    ...
    if "splits" not in protocol:
        raise ValueError("protocol must define a splits mapping")
    if args.split not in protocol["splits"]:
        raise KeyError(f"unknown protocol split: {args.split}")
    specification = protocol["splits"][args.split]
    if not specification.get("open_now", False) and not args.allow_closed:
                ...
                public_rows.append(
                    {
                        ...
                        "split": args.split,
                        "seed_block": specification.get("seed_block"),
                        "seed": seed,
