...
def main() -> None:
    ...
    for split in ("train", "calibration"):
        stem = "prototype_train" if split == "train" else "conformal_calibration"
        parser.add_argument(f"--{split}-snapshots", type=Path, default=ROOT / f"data/piu_v1/{stem}_v3.jsonl")
        parser.add_argument(f"--{split}-labels", type=Path, default=ROOT / f"data/piu_v1/labels/{stem}_v3.jsonl")
        parser.add_argument(f"--{split}-scenes", type=Path, default=ROOT / f"data/piu_v1/{stem}_objects_v3.jsonl")
        parser.add_argument(f"--{split}-object-features", type=Path, default=ROOT / f"data/piu_v1/{stem}_object_features_v3.npz")
        parser.add_argument(f"--{split}-prefix-features", type=Path, default=ROOT / f"outputs/piu_v1/{stem}_v3/pi05_prefix_features.npz")
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "results/models/piu_object_sidecar_v1.pt")
    parser.add_argument("--report", type=Path, default=ROOT / "results/training/piu_object_sidecar_v1.json")
    ...
    if args.output.exists() or args.report.exists():
        raise FileExistsError("PIU V1 model/report are immutable")
