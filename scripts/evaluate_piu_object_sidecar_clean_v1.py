...
from train_piu_object_sidecar_v1 import (  # noqa: E402
    entropy,
    load_split,
    prediction_sets,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--gates",
        type=Path,
        default=ROOT / "benchmarks/piu_v1/clean_evaluation_gates_v1.yaml",
    )
    parser.add_argument(
        "--model", type=Path, default=ROOT / "results/models/piu_object_sidecar_v1.pt"
    )
    parser.add_argument(
        "--snapshots", type=Path, default=ROOT / "data/piu_v1/clean_scene_disjoint_v4.jsonl"
    )
    parser.add_argument(
        "--labels", type=Path, default=ROOT / "data/piu_v1/labels/clean_scene_disjoint_v4.jsonl"
    )
    parser.add_argument(
        "--scenes", type=Path, default=ROOT / "data/piu_v1/clean_scene_disjoint_objects_v4.jsonl"
    )
    parser.add_argument(
        "--object-features",
        type=Path,
        default=ROOT / "data/piu_v1/clean_scene_disjoint_object_features_v4.npz",
    )
    parser.add_argument(
        "--prefix-features",
        type=Path,
        default=ROOT / "outputs/piu_v1/clean_scene_disjoint_v4/pi05_prefix_features.npz",
    )
    parser.add_argument(
        "--frontend-audit",
        type=Path,
        default=ROOT / "results/audits/piu_v1_clean_object_frontend_v4.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "results/calibration/piu_object_sidecar_clean_v1.json",
    )
