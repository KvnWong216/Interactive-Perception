#!/usr/bin/env python3
"""Execute one prospectively frozen S03 public-v2 logical record.

Validation and dry-run modes never invoke inference or write files.  A future
outcome-bearing operation requires ``--allow-outcome-write`` and writes only
to the distinct v2 single-use ledger.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=V2_OUTPUT_ROOT)
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--validate-only", action="store_true")
    mode.add_argument("--dry-run", action="store_true")
    mode.add_argument(
        "--adjudicate-interrupted-as-infrastructure-failure", action="store_true"
    )
...
def main() -> None:
    ...
    plan, runner_identity, model_identity, schedule, request = (
        validate_s03_v2_runner_preflight(
            plan_path=plan_path,
    ...
    canonical_output = _resolve(Path(runner_identity["output"]["root"]))
    if output_root.resolve() != canonical_output.resolve():
        raise ValueError("S03 v2 output root must be the frozen canonical directory")
    schedule_path = _resolve(Path(plan["parent_logical_schedule"]["path"]))
    manifest_path = _resolve(Path(plan["logical_manifest"]["path"]))
    model_identity_path = _resolve(Path(runner_identity["model_identity"]["path"]))
    ...
    summary = {
        "runner_status": "FROZEN_BEFORE_S03_V2_OUTCOMES",
        ...
        "parent_schedule_sha256": sha256(schedule_path),
        "model_identity_sha256": sha256(model_identity_path),
        "ledger": ledger,
    ...
    if not args.allow_outcome_write:
        raise PermissionError(
            "outcome-bearing S03 v2 operation requires --allow-outcome-write"
        )
    ...
    if args.adjudicate_interrupted_as_infrastructure_failure:
        if ledger["in_flight"] != args.execution_index or not started_path.is_file() or closed_path.exists():
            raise ValueError("only the current unclosed S03 v2 index may be adjudicated")
        ...
        record = build_record_artifact(
            ...
            infrastructure_failure={
                ...
                "message": "started v2 execution did not produce a valid immutable record artifact",
        ...
        validate_s03_outcome_schema(record, repository_root=ROOT, identity=model_identity)
        write_record_and_close(
