...
def frozen_v2() -> tuple[dict, dict, dict]:
    return validate_s03_v2_execution_plan(
        ROOT / V2_PLAN_PATH, repository_root=ROOT
    )
...
def test_v2_plan_mutation_fails_closed(
    tmp_path: Path, frozen_v2: tuple[dict, dict, dict]
) -> None:
    plan, _, _ = frozen_v2
    changed = copy.deepcopy(plan)
    changed["logical_record_binding"]["ordered_record_ids"][0] = "replacement"
    path = tmp_path / "changed-plan.json"
    path.write_text(json.dumps(changed))
    with pytest.raises(ValueError, match="deterministic 620-record binding"):
        validate_s03_v2_execution_plan(path, repository_root=ROOT)


def test_v2_dry_run_is_zero_write_and_zero_inference() -> None:
    ...
    assert report["execution_version"] == "s03_public_v2"
    assert report["inference_executed"] is False
    assert report["outcome_present"] is False
    assert report["files_written"] is False
    assert report["policy_request"]["online_oracle_inputs"] == []
    assert _snapshot(output_root) == before == []
    assert not (ROOT / V2_CERTIFICATE_PATH).exists()


def test_dag_v3_keeps_outcome_gate_incomplete() -> None:
    report = evaluate_empirical_dag_v3(
        ROOT / "configs/experiments/piu_empirical_stage_dag_v3.yaml",
        repository_root=ROOT,
    )
    assert report["status"] == "INCOMPLETE"
    assert report["s03_v1"]["status"] == "SEALED_INFRASTRUCTURE_BLOCKED"
    assert report["s03_v2_runner"]["status"] == "FROZEN_READY_BEFORE_S03_V2_OUTCOMES"
    assert report["s03_v2_runner"]["outcomes_generated"] == 0
