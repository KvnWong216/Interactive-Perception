...
|---|---|---:|---:|
| conformal error `0.10` | desired set coverage | no | through the frozen conformal set |
...
| return pose `0.012 m`, `0.08 rad` | controller completion | frozen before collection | controller only |
...
timeouts are counted as failures. The paper reports the preregistered cost
...
Zero target pixels do not by themselves imply `EMPTY`. The temporal evaluator also
requires same-camera-pose counterfactual target visibility, an open searched
region, and completed return-to-observe. This is a logical evidence contract,
